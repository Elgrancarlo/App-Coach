...
function normalizeMessage(message: RawMessage, index: number): GenesisMessage | null {
  ...
  if (content.includes('"type":"image_data"')) {
    return null;
  }
...
export function GenesisUIProvider({ children }: { children: React.ReactNode }) {
  ...
  const sendMessage = useCallback(
    async (content: string, imageData?: { base64: string; mediaType: string }) => {
      if (!token) return;
      ...
      try {
        const res = await fetch(`/api/threads/${targetThreadId}/stream`, {
          ...
          body: JSON.stringify({ content, model: activeModelId, useTavily: activeUseTavily, imageData }),
        });
        ...
        const updateStreamMessage = (text: string) => {
          if (!streamMessageId) return;
          const id = streamMessageId;
          setMessagesBySession((prev) => {
            const existing = prev[targetThreadId] ?? [];
            return {
              ...prev,
              [targetThreadId]: existing.map((msg) => (msg.id === id ? { ...msg, content: text } : msg)),
            };
          });
        };
